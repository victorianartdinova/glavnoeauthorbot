...
from handlers import lot_handler, content_handler, metrics_handler, client_handler, plan_handler, journal_handler, meme_handler, focus_handler, voice_handler, dev_handler, leadgen_cards_handler, package_lot_handler, retro_handler
...
retro_handler.register_handlers(dp)
...
async def main():
    ...
    # Инициализируем и запускаем планировщик для анализа победителей
    try:
        from utils.scheduler import init_scheduler, start_scheduler
        import asyncio

        event_loop = asyncio.get_event_loop()
        init_scheduler(event_loop=event_loop)
        await start_scheduler()
        logger.info("✅ Планировщик запущен")
    except ImportError:
        logger.warning("⚠️ APScheduler не установлен. Автоанализ отключен.")
    except Exception as e:
