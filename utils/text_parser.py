...
def _is_usp(line: str) -> bool:
    ...
    Более строгие критерии для избежания 'воды' (лишнего контента):
    - Должна быть либо явно отмечена буллетом/номером
    - Либо содержать конкретное преимущество (не просто любое слово)
    """
    line_stripped = line.strip()

    # Должна начинаться с буллета/номера чтобы считаться УТП
    if not re.match(r"^[\-\•\–\*\d\)]\s*", line_stripped):
        return False

    # Слишком длинная строка вероятно не УТП
    if len(line_stripped) > 150:
        return False

    # Пустой текст после символа не УТП
    text_after_bullet = re.sub(r"^[\-\•\–\*\d\)]\s*", "", line_stripped)
    if not text_after_bullet or len(text_after_bullet) < 3:
        return False

    # Конкретные ключевые слова УТП (более строгий список)
    specific_usp_keywords = [
        "район", "школ", "сад", "парк", "набережная", "благоустройство",
        "сдача", "ремонт", "класс", "вид", "инфраструктура", "транспорт",
        "ипотека", "рассрочка", "скидка", "акция", "ключ", "готов",
        "террас", "панорам", "консьерж", "паркинг", "подземн"
    ]

    text_lower = text_after_bullet.lower()
    if any(kw in text_lower for kw in specific_usp_keywords):
        return True

    # Исключаем очевидно не-УТП содержимое
    exclude_patterns = [
        r"^(и|или|но|то|что|как|вот|это|будет|есть|может)",  # служебные слова
        r"^(кс|хх|http|www|\[|\()",  # технические артефакты
        r"^(название|город|область|стоимость|цена|метро)",  # meta-информация
    ]
...
def _clean_usp(line: str) -> str:
    """Очищает УТП от лишних символов и 'воды'.

    Удаляет:
    - Буллеты и номера
    - Дополнительные пробелы и переносы
    - Заключительные точки/запятые если это просто пунктуация
    """
    # Убираем буллеты, номера и пробелы
    line = re.sub(r"^[\-\•\–\*\d\)\.]\s*", "", line).strip()

    # Убираем дублирующиеся пробелы
    line = re.sub(r"\s+", " ", line)

    # Убираем завершающую пунктуацию если она в конце (кроме вопросов)
