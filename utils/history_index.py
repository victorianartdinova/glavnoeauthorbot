...
def extract_topics_from_text(text: str) -> List[str]:
    ...
    Более строгие критерии для избежания добавления 'воды'.

    Returns:
        Список ключевых слов/тем (только релевантные)
    """
    topics = []

    # ТОЛЬКО явные темы (не общие слова)
    patterns = {
        "рассрочка": r"рассрочк[аиуе](?:\s*0)?",
        "ипотека": r"ипотек[аиуе]",
        "инвестиция": r"инвестиц|доходност",
        "локация": r"метро|минут[аыуе]?\s+до|район",
        "премиум": r"премиум|элит(?:ный|ных|ном)|люкс",
        "скидка": r"скидк[аиуе]\s+\d+|акци[яию]",
        "апартаменты": r"апартамент[аы]",
        "квартира": r"квартир[аыеу]",
        "пентхаус": r"пентхаус",
        "видовой": r"панорам|вид[аыеу]\s+на",
        "готовность": r"готов[аыеи]|ключи|сдач[аи]",
        "офис": r"офис[аыеов]",
    }
...
def extract_hook_type(text: str) -> str:
    ...
    Более строгие критерии для избежания 'воды' при определении типов.
    ...
    # Проверяем основной хук - отбираем только сильные сигналы
    # Чтобы избежать ложных срабатываний, требуем явные указания

    # ФИНАНСОВЫЙ хук — явные условия платежей
    financial_patterns = [
        r'рассрочк[аиуе]',  # рассрочка
        r'ипотек[аиуе]',    # ипотека
        r'первый\s+взнос|пв[:\s]',  # первый взнос
        r'платёж[:\s]|платеж[:\s]',  # платёж
        r'от\s+\d+\s*млн',  # от N млн
    ]
    if any(re.search(p, text_lower) for p in financial_patterns):
        return "financial"

    # ЛОКАЦИОННЫЙ хук — явные упоминания метро/района
    location_patterns = [
        r'м\.\s+[А-Яа-яЁё]+',  # м. Станция
        r'метро\s+[А-Яа-яЁё]+',  # метро Станция
        r'\d+\s*мин[а-я]*\s+до\s+метро',  # 5 минут до метро
        r'район[:\s]+[А-Яа-яЁё]+',  # район: название
    ]
    if any(re.search(p, text_lower) for p in location_patterns):
        return "location"

    # ПРЕМИУМ хук — элитность/статус (осторожно, чтобы не было воды)
    premium_patterns = [
        r'премиум[\s-]*класс',
        r'элит[\s-]*класс|элитн',
        r'люкс',
        r'бизнес[\s-]*класс',
    ]
    if any(re.search(p, text_lower) for p in premium_patterns):
        return "premium"

    # СРОЧНОСТЬ — реальное ограничение времени
    urgency_patterns = [
        r'осталось\s+\d+',  # осталось N (квартир/мест/дней)
        r'последн[иеяй]+\s+квартир',  # последние квартиры
        r'только до\s+\d+',  # только до даты
        r'успей\s+до',  # успей до
    ]
    if any(re.search(p, text_lower) for p in urgency_patterns):
        return "urgency"

    # ЭМОЦИОНАЛЬНЫЙ хук — образ жизни (только явные)
    emotional_patterns = [
        r'мечта',  # мечта, мечтали
        r'образ\s+жизни',
        r'представ[ьи]',  # представь, представьте
    ]
...
def extract_angle(text: str) -> str:
    ...
    # Проверяем углы в порядке приоритета (более специфичные сначала)
    angles = [
        ("рассрочка 0%", r"рассрочк[аиуе]\s*0\s*%"),
        ("ипотека льготная", r"льготн[аяое]\s+ипотек"),
        ("локация метро", r"\d+\s*мин[а-я]*\s+до\s+метро"),
        ("локация центр", r"центр\s+(?:москвы|города)|красн[аяое]+\s*площад"),
        ("вид панорама", r"панорам\w+\s+(?:вид|окн)|вид[аыеу]\s+на\s+(?:город|парк|воду)"),
        ("готовность", r"ключи\s+(?:сразу|после)|готов[аыеи]\s+(?:к\s+)?(?:заселению|проживанию)"),
        ("последние", r"осталось\s+\d+|последн[иеяй]+\s+квартир"),
        # Менее специфичные (проверяем в конце)
        ("рассрочка", r"рассрочк[аиуе]"),
        ("ипотека", r"ипотек[аиуе]"),
        ("скидка", r"скидк[аиуе]\s+(?:\d+|млн)"),
        ("инвестиция", r"инвестиц|доходност"),
    ]

    for angle, pattern in angles:
        if re.search(pattern, text_lower):
            return angle
